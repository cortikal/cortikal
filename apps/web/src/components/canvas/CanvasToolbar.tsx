    ...
    <div className={styles.toolbar} id="canvas-toolbar">
      ...
      <div className={styles.group}>
        ...
        <button
          className={styles.btn}
          onClick={async () => {
            if (nodes.length === 0 || isDeploying) return;
            setIsDeploying(true);
            try {
              // 1. Serialize the canvas graph to arch.md
              const graph = reactFlowToArch(nodes, edges);
              const archDocument = {
                metadata: metadata ?? {
              ...
              const archMdContent = await ApiClient.serializeArchMd(archDocument);
              
              // 2. Create a project
              const project = await ApiClient.createProject(
                metadata?.name || "Untitled Project",
                metadata?.description || "Generated by Cortikal Swarm"
              );
              
              // 3. Start the swarm
              await ApiClient.swarm.start(project.id, archMdContent);
              
              // 4. Navigate to the swarm page
              router.push(`/swarm?projectId=${project.id}`);
            } catch (err) {
              console.error("Failed to deploy to swarm:", err);
              alert("Failed to deploy. Is the backend running?");
            } finally {
              setIsDeploying(false);
            }
          }}
          title="Deploy to Swarm — AI agents will build this architecture"
          id="toolbar-deploy-swarm"
          disabled={nodes.length === 0 || isDeploying}
          style={nodes.length > 0 ? { background: "var(--color-accent)", color: "#fff" } : {}}
        >
          {isDeploying ? "⏳" : "🚀"}
        </button>
      </div>
